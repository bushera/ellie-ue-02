    ...
    async function fetchBookings() {
      ...
      data.records.forEach((record) => {
        ...
        if (booking.status === 'ACCEPTED') {
          ...
          itemDiv.innerHTML = `
            <h3>${booking.title}</h3>
            <p><b>Timing:</b> ${booking.start_date} - ${booking.end_date}</p>
            <p><b>Location:</b> ${booking.location}</p>
            <a href="${booking.cal_url}" target="_blank">Click to Join</a><br><br>
            <button class="cancel" data-id="${booking.booking_id}">Cancel</button>
            <button class="reschedule" data-id="${booking.booking_id}">Reschedule</button>
          `;
          carouselInner.appendChild(itemDiv);
        } else {
          engagedExists = true;
          summary[booking.status.toLowerCase()]++;
        }
      });

      if (!activeExists) activeSection.style.display = 'none';
      if (!engagedExists) engagedSection.style.display = 'none';
      else {
        engagedContainer.innerHTML = `
          <p>Missed: ${summary.missed}</p>
          <p>Cancelled: ${summary.cancelled}</p>
      ...
      btnLeft.onclick = () => carouselInner.scrollBy({ left: -270, behavior: 'smooth' });
      btnRight.onclick = () => carouselInner.scrollBy({ left: 270, behavior: 'smooth' });
    }
  }
};
