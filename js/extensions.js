  ...
  render: async ({ trace, element }) => {
    ...
    container.innerHTML = `
      ...
      <style>
        ...
        #active-section, #engaged-section {
          margin-bottom: 15px;
        }
        .call {
          border: 1px solid #ddd;
        ...
        .carousel {
          margin: 5% !important; /* 5% margin around carousel */
        }
        .carousel-item {
          padding: 15px 10px;
        ...
        /* Position carousel controls vertically centered on the sides */
    ...
    async function fetchBookings() {
      ...
      try {
        ...
        const carouselId = 'activeBookingsCarousel';
        activeContainer.innerHTML = `
          <div id="${carouselId}" class="carousel slide" data-bs-ride="carousel" data-bs-interval="false" data-bs-touch="true">
        ...
        const carouselInner = activeContainer.querySelector('.carousel-inner');
        ...
        data.records.forEach((record, index) => {
          ...
          if (booking.status === 'ACCEPTED') {
            ...
            itemDiv.innerHTML = `
              <h3 style="word-wrap: break-word;">${booking.title}</h3>
              <p style="word-wrap: break-word; margin: 5% 0;">${formattedStart} - ${formattedEnd} • ${booking.location}</p>
              <a href="${booking.url}" style="word-wrap: break-word; display:block; margin-bottom: 5%;">Click to Join call via •${booking.url}</a>
              <button class="cancel btn btn-dark mt-2 w-100" data-id="${booking.bookingId}" data-title="${booking.title}">Cancel</button>
              <button class="reschedule btn btn-success mt-2 w-100" data-id="${booking.bookingId}" data-title="${booking.title}">Reschedule</button>
            `;
